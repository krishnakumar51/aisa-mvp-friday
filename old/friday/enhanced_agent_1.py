...
class Agent1Scratchpad:
    ...
    def __init__(self):
        ...
        self.insights: List[Dict[str, Any]] = []  # For compatibility
    ...
    def add_insight(self, text: str, ctx: Dict[str, Any] = None):
        """Add insight for compatibility with manager"""
        self.insights.append({"text": text, "ctx": ctx or {}, "ts": time.time()})
...
# CRITICAL FIX: Accept 5 arguments to match manager call
def run_enhanced_agent1(seq_no: str, pdf_path: Union[str, Path], instructions: str, platform: str, images: Optional[List[str]] = None) -> dict:
    """
    Enhanced Agent 1 with advanced blueprint generation, caching, and reflection capabilities.
    
    FIXED SIGNATURE: Now accepts 5 arguments to match manager call:
    run_enhanced_agent1(seq_no, pdf_path, instructions, platform, images)
    """
    
    # Convert pdf_path to Path if string
    ...
    print(f"[{seq_no}] 📄 PDF: {pdf_path.name}")
    print(f"[{seq_no}] 🎯 Platform: {platform}")
    print(f"[{seq_no}] 📝 Instructions: {instructions[:100]}..." if len(instructions) > 100 else f"[{seq_no}] 📝 Instructions: {instructions}")
    print(f"[{seq_no}] 🖼️ Images: {len(images)} provided")
    ...
    try:
        out_dir = Path(ARTIFACTS_DIR) / seq_no / "enhanced_agent1"
        out_dir.mkdir(parents=True, exist_ok=True)
...
"""
        ...
        blueprint_dict["metadata"].update({
            ...
            "agent_version": "enhanced_1.1_signature_fixed",
        ...
        print(f"[{seq_no}] ⏱️ Execution time: {execution_time:.2f}s")
        print(f"[{seq_no}] 📁 Blueprint saved: {blueprint_path}")
        ...
        error_msg = str(e)
        print(f"[{seq_no}] ❌ Enhanced Agent 1 failed: {error_msg}")
        raise HTTPException(status_code=500, detail=f"Enhanced Agent 1 execution failed: {error_msg}")

def run_agent1(seq_no: str, pdf_path: Path, instructions: str, platform: str) -> dict:
    """Backward compatible wrapper for enhanced agent 1 - EXACT FUNCTION SIGNATURE PRESERVED"""
...
def get_agent1_cache_stats() -> Dict[str, Any]:
    """Get cache statistics for monitoring"""
    return agent1_cache.get_stats()

def clear_agent1_cache() -> None:
    """Clear cache for troubleshooting"""
    agent1_cache._cache.clear()
    agent1_cache.hit_count = 0
    agent1_cache.miss_count = 0

def get_agent1_insights() -> List[Dict[str, Any]]:
    """Get recent insights for debugging"""
    return agent1_scratchpad.reflection_log[-10:]  # Last 10 insights

def get_agent1_scratchpad_stats() -> Dict[str, Any]:
    """Get scratchpad statistics"""
    return {
        "content_analysis_cached": len(agent1_scratchpad.content_analysis_cache),
        "image_analysis_cached": len(agent1_scratchpad.image_analysis_cache),
        "reflection_entries": len(agent1_scratchpad.reflection_log),
        "processed_patterns": len(agent1_scratchpad.processed_patterns)
    }
...
__all__ = [
    "run_enhanced_agent1", 
    "run_agent1", 
    "analyze_pdf_content_smart", 
    "enhance_image_analysis_smart", 
    "agent1_cache", 
    "agent1_scratchpad",
    "get_agent1_cache_stats",
    "clear_agent1_cache", 
    "get_agent1_insights",
    "get_agent1_scratchpad_stats",
